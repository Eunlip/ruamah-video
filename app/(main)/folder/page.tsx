...
function VideoCard({ video, onPlay, onLongPress }: { video: VideoItem, onPlay: () => void, onLongPress: () => void }) {
    const longPressProps = useLongPress(() => onLongPress(), () => onPlay());
        ...
        >
            ...
            <div className="absolute bottom-0 left-0 right-0 p-5">
                ...
                <p className="text-[13px] text-white/80 font-medium flex items-center gap-1.5">
                    <Video size={14} />
                    Diunggah: {video.createdAt.toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}
                </p>
            </div>
        ...
        setTimeout(() => {
            ...
            if (videoEl) {
                if (videoEl.requestFullscreen) {
                    videoEl.requestFullscreen().catch(e => console.log("Fullscreen failed:", e));
                    ...
                    {filteredVideos.length === 0 ? (
                        <div className="text-center py-12 px-4 bg-surface-container-lowest rounded-[20px] border border-surface-dim/50 border-dashed">
                            ...
                            <p className="text-sm text-on-surface-variant">
                                {searchQuery ? `Tidak ada video dengan nama "${searchQuery}"` : 'Upload video pertama untuk folder ini.'}
                            </p>
                        </div>
                    ) : (
                        <div className="flex flex-col gap-4">
                            {filteredVideos.map(video => (
                                <VideoCard 
                                    ...
                                    onLongPress={() => setActionSheetVideo(video)} 
                                />
                            ))}
                        </div>
                    )}
...
export default function FolderDetailPage() {
    return (
        <Suspense fallback={<div className="min-h-screen bg-surface flex items-center justify-center text-on-surface-variant">Memuat...</div>}>
            <FolderDetailContent />
        </Suspense>
    );
}
