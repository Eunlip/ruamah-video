...
function FolderHeader({
    ...
    return (
        ...
        >
            <div
                className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 bg-gradient-to-br ${
                    isKpi
...
function YearChip({ year, isKpi, onClick, onLongPress }: YearChipProps) {
    ...
    return (
        <div
            {...props}
            className={`relative flex items-center gap-2 px-4 py-2.5 rounded-xl cursor-pointer transition-all active:scale-95 border select-none ${
                isKpi
                    ? "bg-primary/5 border-primary/20 hover:bg-primary hover:text-white text-primary"
                    : "bg-secondary/5 border-secondary/20 hover:bg-secondary hover:text-white text-secondary"
            }`}
        >
            <Calendar size={16} />
...
export function FolderView({ title, type, initialFolders }: FolderViewProps) {
    ...
    const [actionSheetYear, setActionSheetYear] = useState<{
        category: string;
        year: string;
    } | null>(null);
    ...
    const handleDeleteYear = (categoryName: string, year: string) => {
        setFolders((prev) =>
            prev.filter((f) => !(f.name === categoryName && f.year === year)),
        );
    };
                    ...
                    {isLoading ? (
                        ...
                        groupedFolders.map((group) => {
                            ...
                            return (
                                <div
                                    key={group.id}
                                    className={`bg-surface-container-lowest border rounded-[20px] overflow-hidden shadow-sm transition-all duration-300 ${
                                        isExpanded
                                            ? "border-primary/30 shadow-md ring-2 ring-primary/10"
                                            : "border-surface-dim hover:border-primary/20 hover:shadow-md"
                                    }`}
                                >
                                    {/* Main Folder Header */}
                                    <FolderHeader
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {group.years.map((year) => (
                                                    <YearChip
                                                        ...
                                                        onClick={() =>
                                                            router.push(
                                                                `/folder?category=${encodeURIComponent(
                                                                    group.name,
                                                                )}&year=${year}&type=${type}`,
                                                            )
                                                        }
                    ...
                    <div className="relative w-full max-w-md bg-surface-container-lowest rounded-t-[32px] sm:rounded-[32px] shadow-2xl overflow-hidden animate-in slide-in-from-bottom-full sm:slide-in-from-bottom-8 duration-300">
                        ...
                        >
                            ...
                            <Input
                                ...
                                value={newFolderYear}
                                onChange={(e) =>
                                    setNewFolderYear(e.target.value)
                                }
                            />
            ...
            {addingYearTo && (
                <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-5">
                    ...
                    <div className="relative w-full max-w-md bg-surface-container-lowest rounded-t-[32px] sm:rounded-[32px] shadow-2xl overflow-hidden animate-in slide-in-from-bottom-full sm:slide-in-from-bottom-8 duration-300">
                        ...
                        <form
                            onSubmit={handleAddYearOnly}
                            className="p-6 flex flex-col gap-5"
                        >
                            <p className="text-sm text-on-surface-variant font-sans">
                                Menambahkan tahun baru untuk kategori:
                                <br />
                    ...
                    <div className="relative w-full max-w-md bg-surface-container-lowest rounded-t-[32px] p-6 pb-12 animate-in slide-in-from-bottom-full duration-300 shadow-2xl">
                        ...
                        <div className="flex flex-col gap-3">
                            <button
                                onClick={() => {
                                    handleDeleteYear(
                                        actionSheetYear.category,
                                        actionSheetYear.year,
                                    );
